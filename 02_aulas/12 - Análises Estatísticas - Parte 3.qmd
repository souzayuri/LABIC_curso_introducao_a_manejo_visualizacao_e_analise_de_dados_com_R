...
::: {style="font-size: 70%"}
Baseado na equação geral da reta:
...
-   Relação **linear** entre preditoras e resposta
    ...
    -   `duas ou mais` = **regressão linear múltipla**
...
-   `Estatística F` (F = 58.84): modelo linear é significativamente **melhor do que** um modelo **sem preditor;** F muito grande (ex.: 10, 50, 100…)
:::
...
-   `Estatística F` (F = 30.63): modelo linear é significativamente **melhor do que** um modelo **sem preditor;** F muito grande (ex.: 10, 50, 100…)
:::
...
Não há `ε` explícito, a distribuição de `Y` é definida (família: `binomial`, `poisson`...)
...
Alguns pressupostos são **os mesmos** para **diferentes** famílias de distribuição, mas outros **podem mudar**.
...
-   `p = 0.984`: O modelo não apresenta problemas de dispersão.
:::
...
1.  **Tipo de variáveis?**
    ...
    -   Continua vs Categórica (2 grupos) → **t-test**
...
::: {.columns style="font-size: 60%"}
1.  Faça uma regressão linear usando os dados **`riqueza_lais`.csv**

    -   **Responda**: A riqueza de espécies aumenta ou diminui em função da cobertura vegetal?

        -   chequem todos os pressupostos para saber se o modelo é confiável
...
```{r}
dados_riqueza <- read_csv("../01_dados/riqueza_lais.csv")

dados_riqueza_lm <- lm(Richness_total ~ Forest_cover, data = dados_riqueza)

summary(dados_riqueza_lm)
...
```{r}
shapiro.test(dados_riqueza_lm$residuals)
...
#| fig-width: 6
#| fig-height: 6
performance::check_model(dados_riqueza_lm) 
```
:::
:::::
::::::
...
lmtest::bptest(dados_riqueza_lm)
...
#| fig-height: 3.5
#| output-location: column
ggplot(dados_riqueza, aes(x = Forest_cover, y = Richness_total)) +
...
::: {.columns style="font-size: 60%"}
1.  Faça uma regressão linear usando os dados **`exercicio_glm`.csv**

    -   **Responda**: A riqueza de espécies aumenta ou diminui em função da cobertura florestal e elevação?
...
1.  Faça uma regressão linear usando os dados **`exercicio_glm.csv`**
    ...
    -   **Responda**: A riqueza de espécies aumenta ou diminui em função da altitude e cobertura florestal?
...
```{r}
dados_glm <- read_csv("../01_dados/exercicio_glm.csv")
```
:::
...
```{r}
#| output-location: column
exercicio_poisson <- glm(
  n_especies_mamiferos ~ cobertura_floresta + elevacao,
  family = poisson(link = "log"),
  data = dados_glm)

summary(exercicio_poisson)
...
#| fig-width: 6
#| fig-height: 4
simulacao <- DHARMa::simulateResiduals(exercicio_poisson, plot = TRUE)
```
:::
...
modelo_null <- glm(n_especies_mamiferos ~ 1, 
                   family = poisson, 
                   data = dados_glm)

anova(modelo_null, exercicio_poisson)
```
:::
...
```{r}
fmsb::NagelkerkeR2(exercicio_poisson)
```
:::
...
#| fig-height: 3.5
#| output-location: column
sjPlot::plot_model(exercicio_poisson, 
                   type = "pred", 
                   terms = c("cobertura_floresta", "elevacao [0, 500, 1000, 1500]"),
                   title = "Efeitos Marginais")
```
:::
...
```{r}
report::report(exercicio_poisson)
...
Email: [desouza.s.yuri\@gmail.com](mailto:desouza.s.yuri@gmail.com)

Website: [souzayuri.github.io](https://souzayuri.github.io/)

**Materiais do curso:**

[GitHub](https://github.com/souzayuri/LABIC_curso_introducao_a_manejo_visualizacao_e_analise_de_dados_com_R)

![](img/starbucks-coffee.gif){fig-align="center" width="250"}
:::
