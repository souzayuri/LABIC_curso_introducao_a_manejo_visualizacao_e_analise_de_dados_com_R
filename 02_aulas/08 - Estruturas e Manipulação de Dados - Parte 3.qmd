...
::: {style="font-size: 70%"}
-   O R apresenta **pacotes**, que são conjuntos **de funções**.
...
-   Analogia: `produção de uma bebida`
    ...
    -   O efeito **acaba na última função** à qual não tem mais nenhum `%>%` associado
:::
...
Ex: adicionar o número das linhas com `ID` através da função `row_number()`
...
-   Pega um **conjunto de `colunas`** e **transforma** em `linhas`
    ...
    -   `Outra coluna` com valores que existiam nas células
...
-   Pega uma **`coluna`** que contém **nome de variáveis** e **transforma** cada uma em uma `nova coluna`
    -   `Uma coluna` contendo o nome das colunas a serem criadas

    -   `Outra coluna` com valores que são atribuídos as novas colunas e células
...
-   Pega uma **`coluna`** que contém **nome de variáveis** e **transforma** cada uma em uma `nova coluna`
    ...
    -   `Outra coluna` com valores que são atribuídos às novas colunas e células
-   Formato **mais usado** em `análises` que requerem formato de `matrix`
:::
...
-   **Manter todas as linhas da tabela à `esquerda` e adicionar as da `direita.`**
...
-   Calcule o número **total de interações** por **espécie** e **tipo de interação** `(interaction)`. Dica: `group_by()`, `summarise()` e `sum()`
