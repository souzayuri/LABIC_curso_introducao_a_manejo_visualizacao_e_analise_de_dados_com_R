...
1.  **Normalidade**: os dados seguem uma distribuição normal? Os valores se concentram no meio e diminuem de forma simétrica para os extremos.
:::
...
2.  **Homogeneidade de variâncias** (homocedasticidade): as variâncias são iguais entre os grupos? Os grupos têm níveis parecidos de variabilidade, mesmo que as médias sejam diferentes.
:::
...
::: {style="font-size: 70%"}
Estatísticas `não paramétricas` exigem:
...
::: {style="font-size: 50%"}
Discreta, com número finito de resultados possíveis, pode ser **simétrica ou assimétrica**; depende dos parâmetros. Descreve o **número de sucessos** em *n* tentativas independentes, cada uma com a mesma probabilidade de sucesso *p*.
...
-   Testar se as **frequências observadas** são significativamente **diferentes** das **esperadas**
...
**Degrees of Freedom (`df`):** quantidade de informação independente disponível para **estimar a variabilidade** (**`1-N`**).
:::
:::::
::::::
...
-   `H0`: os dados seguem distribuição normal (fazer para ambas as variaveis)
...
::: {style="font-size: 60%"}
-   Todos os **pressupostos foram atendidos**, seguimos com `Student's t-test`

    -   se qualquer pressuposto fosse violado, teríamos de usar `Welch's t-test`
...
::: {style="font-size: 70%"}
Testando a **homogeneidade** de variâncias **das matrizes**
...
Os dados **são normais** e a **variabilidade** é semelhante entre grupos; podemos seguir!
::::::
...
-   **Tukey HSD**: ***Honestly Significant Difference*** é usado para identificar quais pares de **médias diferem entre si**, controlando o erro do Tipo I.
