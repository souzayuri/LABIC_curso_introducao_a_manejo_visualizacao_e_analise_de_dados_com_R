---
title: "Análises Estatísticas no R<br>"
...
-   **Ferramentas**: Médias, desvios, gráficos, correlações
...
-   **Exemplo**: "Cada 1°C aumenta em 12% a abundância de mosquitos"
:::
...
::: {style="font-size: 70%"}
```{r}
# Carregar dados - usaremos iris como exemplo
dados <- iris %>% 
  as_tibble() %>% 
...
-   Resumir e descrever as **características principais** de um conjunto de dados
...
#| fig-width: 10
#| fig-height: 5
ggplot(dados, aes(x = Species, y = Sepal.Length, fill = Species)) +
  geom_boxplot(alpha = 0.7) +
  geom_jitter(width = 0.1, alpha = 0.3) +
  theme_minimal() +
  labs(x = "Espécie", 
       y = "Comprimento da Sépala (cm)",
       title = "Comparação do comprimento da sépala entre espécies") +
...
ggcorrplot(cor_matrix, 
           ...
           title = "Matriz de Correlação - Variáveis Morfométricas")
```
:::
...
cor.test(dados$Sepal.Length, dados$Petal.Length)
```
...
#| fig-width: 10
#| fig-height: 5
ggplot(dados, aes(sample = Sepal.Length)) +
  stat_qq() +
...
```{r}
# Teste de Levene
leveneTest(Sepal.Length ~ Species, data = dados)
```
...
```{r}
# Modelo ANOVA
modelo_anova <- aov(Sepal.Length ~ Species, data = dados)
...
ggplot(dados, aes(x = Species, y = Sepal.Length)) +
...
modelo_lm <- lm(Petal.Length ~ Sepal.Length, data = dados)
summary(modelo_lm)
```
:::
